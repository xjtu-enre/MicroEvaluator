...
const figureData = function (datas) {
  const barData = [];
  const result = datas.children.map((data) => convert(barData, data));
  return [result, barData];
};

const convert = function (count, data) {
  const item = {
    id: data.id.toString(),
    name: data.name,
  };
  if (!data.children) {
    ...
    item.children = [];
    data.children.forEach((e) => item.children.push(convert(count, e)));
  }
  return item;
};

export const drawCluster = function (temp, params) {
  ...
  debugger
  console.log(data);
  const [padding, width, height] = [0, 300, 400];
  ...
  const typeArray = ['Central', 'Core', 'Control', 'Shared', 'Peripheral'],
    colorTypeArray = ['#A85CF9', '#A85CF9', '#5534A5', '#4B78E5', '#6FDFDF'];
  const colorScaleByType = d3.scaleOrdinal().domain(typeArray).range(colorTypeArray);
  const colorBar = d3
    .scaleOrdinal()
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      if (hidden) {
        ...
        chosenContext.fillStyle = node.children ?
          typeArray.indexOf(node.data.name) !== -1 ?
          colorScaleByType(node.data.name) :
          node.data.color :
          'white';
      }
      ...
      chosenContext.fill();
      // 画bar图
      //  node.data.id==undefined  问题可能出在这里
...
export const drawLegend = () => {
  ...
  const create = (axisY, text, depth, colorList, textList) => {
    const textY = 15 + depth;
    gs.append('rect')
      .attr('x', 0)
      .attr('y', axisY)
      .attr('width', 300)
      .attr('height', 90)
      .attr('fill', 'white');
    gs.append('text').attr('x', 0).attr('y', textY).text(text);
    let legendX = 30,
      legendY = 50 + depth;
    const marginX = 20,
      circleR = 20;
  ...
  const colorBarList = ['#247ba0', '#70c1b3', '#b2dbbf', '#f3ffbd', '#ff1654'],
    textBarList = ['<100', '<500', '<1000', '<2000', '>2000'];
  const colorTypeList = ['#A85CF9', '#5534A5', '#4B78E5', '#6FDFDF'],
    textTypeList = ['Central', 'Control', 'Shared', 'Peripheral'];
  create(0, '代码修改频率', 0, colorBarList, textBarList);
  create(100, '聚类类型', 100, colorTypeList, textTypeList);
};
