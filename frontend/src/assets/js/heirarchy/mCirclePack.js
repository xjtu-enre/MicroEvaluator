...
const convert = function (count, data) {
  ...
  // console.log('circlepackitem', item)
  return item;
};
...
export const drawmLegend = function () {
  const svg = d3
    .select('#legend')
    .append('svg')
    .attr('id', 'legendSvg')
  ...
  gs.append('rect').attr('x', 0).attr('width', 250).attr('height', 150).attr('fill', 'white');
  gs.append('text').attr('x', 0).attr('y', 15).text('代码修改频率');
  const colorList = ['#247ba0', '#70c1b3', '#b2dbbf', '#f3ffbd', '#ff1654'];
  const textList = ['<100', '<500', '<1000', '<2000', '>2000'];
  let legendX = 25,
      legendY = 17;
  const marginX = 7,
      circleR = 15;
...
export const drawRootClusters = function (datas) {
  ...
  const textScale = d3
    .scaleOrdinal()
    .domain(colorType)
    .range(['小于100', '小于500', '小于1000', '小于2000', '大于2000']);
