...
const processNodesEdges = (
  ...
  nodes.forEach((node) => {
    ...
    node.oriLabel = node.name ? node.name : node.label;
    node.label = formatText(node.name ? node.name : node.label, labelMaxLength, '...');
    node.degree = 0;
    node.inDegree = 0;
    node.outDegree = 0;
  ...
  originData.edges.forEach((edge) => {
    if (edge.source === extractNodeId) {
      ...
      if (!aggregatedNodeMap[targetClusterId].expanded) {
        // did not expand, create an virtual edge from the extract node to the cluster
...
console.log(getExtractNodeMixedGraph)
...
const bindListener = (graph) => {
  ...
  graph.on('canvas:click', (evt) => {
    console.log(evt)
    clearFocusItemState(graph);
    // console.log(graph.getGroup(), graph.getGroup().getBBox(), graph.getGroup()
    // .getCanvasBBox());
  });
};

export const drawChart = function (data1, data2) {
  const data = {
    nodes: data1.map((e) => ({
      id: e._id.toString(),
      mode: e.mode_type,
  ...
  data.edges.forEach((e) => {
    const sourceMode = tempModeMap[e.source],
      targetMode = tempModeMap[e.target];
    if (tempEdges[`${sourceMode}-${targetMode}`]) {
      tempEdges[`${sourceMode}-${targetMode}`]['count']++;
    } else {
      tempEdges[`${sourceMode}-${targetMode}`] = {
        source: sourceMode,
        target: targetMode,
        count: 1,
      };
  ...
  Object.values(tempEdges).forEach((tempEdge) => {
    clusteredData.clusterEdges.push(tempEdge);
  });
  ...
  //     const datas = e.item.getModel().data;
  //     const liInfos = Object.keys(datas)
  //       .map((d) => (datas[d] !== null ? `<li>${d}:${datas[d]}</li>` : ''))
  //       .join('');
  //     outDiv.innerHTML = `<ul>` + liInfos + `</ul>`;
  //     return outDiv;
  //   },
  //   shouldBegin(e) {
  //     return !e.item.getModel().level;
  //   },
  //   itemTypes: ['node'],
  // });
