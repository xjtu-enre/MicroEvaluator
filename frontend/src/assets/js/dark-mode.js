...
export function activateDarkMode() {
  ...
  const card_border = document.querySelectorAll(".cards.border");
  const mainContent_blur = document.querySelectorAll(
    ".main-content .container-fluid .cards",
  );
...
export function deactivateDarkMode() {
  ...
  for (var i = 0; i < text_span_white.length; i++) {
    if (
      ...
      !text_span_white[i].closest(".cards.bg-gradient-dark")
    ) {
  ...
  for (var i = 0; i < btn_text_white.length; i++) {
    if (!btn_text_white[i].closest(".cards.bg-gradient-dark")) {
