...
#                 tmplist = interCoWei_original(serviceID1, serviceID2, serviceDict, commitDict, repeatClassDict,
#                                               flagDict)
...
def statis_new(serviceDict, commitDict, repeatClassDict):
    ...
    for serviceID1 in serviceDict:
        totalTitle.append(serviceID1)
        totalSize.append(len(serviceDict[serviceID1]))
        ...
        totalIcf_avg.append(avgV)
        icfperservice[serviceID1] = ("%.4f" % avgV)

        # ecf of this service
        # if size == 1, then ecf = 1, it is sured to co-change with others.
        if len(serviceDict) == 1:
            avg_ecf = 0
        elif len(serviceDict[serviceID1]) == 1:
            avg_ecf = 1.0
        else:
            valuelist = list()
            flagDict = dict()  # (class1, class2) in dict,  class2 is repeat class
            for serviceID2 in serviceDict:
                if serviceID1 != serviceID2:
                    tmplist = interCoWei_correct(serviceID1, serviceID2, serviceDict, commitDict, repeatClassDict,
                                                 flagDict)
                    valuelist.extend(tmplist)
            avg_ecf = sum(valuelist) / float(len(valuelist))
        totalecf_avg.append(avg_ecf)
        ecfperservice[serviceID1] = ("%.4f" % avg_ecf)

    summarizeList = [totalTitle, totalSize, totalIcf_avg, totalecf_avg, icfperservice, ecfperservice]
    return summarizeList
...
'''
ioe_measure_method api
'''
def ioe_measure_method(commitFileName, fileType, serviceFileName):
    ...
    icf_dict = summarizeList[4]
    ecf_dict = summarizeList[5]
    ...
    if final_ICF == 0:
        ...
        final_REI = final_ECF / final_ICF
    '''
    print("final_ICF, final_ECF, final_REI:", final_ICF, final_ECF, final_REI)
    print("each service detail, icf_list:", icfList)
    print('each service detail, ecf_list:', ecfList)
    '''
    return "%.4f" % final_ICF, "%.4f" % final_ECF, "%.4f" % final_REI, icf_dict, ecf_dict
