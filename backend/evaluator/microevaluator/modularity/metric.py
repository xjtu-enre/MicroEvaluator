...
def get_call_coup_ForIndiv(indiv):
    ...
    coup_list = list()
    # print(all_candidate_call_coup_dict)
    for index1 in range(0, len(indiv)):
        single_coup_list = list()
        cand1 = uniqueEncode(indiv[index1])
        for index2 in range(0, len(indiv)):
            if index1 == index2:
                continue
            cand2 = uniqueEncode(indiv[index2])
            coup = all_candidate_call_coup_dict[cand1][cand2]
            single_coup_list.append(coup)
        coup_list.append(sum(single_coup_list))
    if len(coup_list) == 1 and coup_list[0] == 0:
        indiv_metric = 0
    elif len(coup_list) != 0:
        indiv_metric = sum(coup_list) / float(pow(len(indiv), 2) - len(indiv))
    else:
        indiv_metric = 0
...
def get_four_metrics(indiv):
    ...
    return call_coh, call_coup, con_coh, con_coup, call_coh_detail, call_coup_detail, con_coh_detail, con_coup_detail
